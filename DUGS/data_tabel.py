"""
Tabel node: read from / write to a dugs Tabel (built-in spreadsheet).
Behaves like the Google Sheets node does in n8n, but against a local Tabel.

params:
  tabel     : name of the Tabel to use
  operation : "read" | "append" | "update"
      read   -> outputs one item per row in the Tabel (ignores input)
      append -> adds each incoming item's json as a new row; outputs the rows added
      update -> for each incoming item that has an "id", updates that row with the
                item's json fields; outputs the updated rows

Stored Tabels live in <dugs>/tabels/. The "id" field is the row number.
"""
import os
import sys
from node_base import Node

# tabel_store lives one level up from nodes/
