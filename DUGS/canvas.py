    ...
    def paintEvent(self, _):
        ...
        for n in self.nodes:
            ...
            if pm is not None:
                ...
                iy = n.y + (n.s - pm.height()) / 2 - 6   # nudge up to leave room for title
                p.drawPixmap(int(ix), int(iy), pm)
                # title just below the icon
                p.setPen(QColor("#ffffff")); f = QFont("monospace"); f.setPointSize(8); f.setBold(True); p.setFont(f)
                p.drawText(QRectF(n.x, n.y + n.s - 30, n.s, 14),
                           Qt.AlignmentFlag.AlignCenter, n.title)
            else:
                # no icon available -> original look: title centered
